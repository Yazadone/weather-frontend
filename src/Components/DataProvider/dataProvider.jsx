...
const DataProvider = ({ children }) => {
  ...
  const fetchWeatherData = async () => {
    try {
      const res = await fetch(
        `http://localhost:5000/api/weather?lat=${selectedCity[0]?.latitude}&lon=${selectedCity[0]?.longitude}`
      );
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(
        `http://localhost:5000/api/geo?city=${searchData}`
      );
      const data = await res.json();
      console.log("Data", data);
      const cities = data?.map((item, index) => {
        //Saving the city object from data into cities variable
        return {
          id: index,
