...
test('Dropdown renders correct options based on user input using onChange directly', async () => {
    ...
    const mockSelectedCity = [
        { id: 1, name: 'New York, New York, US', latitude: 40.7128, longitude: -74.006 },
    ];
    const mockCityName = [
        { id: 1, name: 'New York, New York, US', latitude: 40.7128, longitude: -74.006 },
    ];
    ...
    render(
        <DataContext.Provider value={{ searchData: 'New York', cityName: mockCityName, selectedCity: mockSelectedCity, setselectedCity: mocksetSelectedCity }}>
            <SearchBar />
    ...
    expect(fetch).toHaveBeenCalledWith("http://localhost:5000/api/weather?lat=40.7128&lon=-74.006");
